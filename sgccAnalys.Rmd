...
groups=read_tsv("Groups_per_component.tsv",show_col_types = FALSE)
groups=groups%>%filter(subtype=="Her2")
groups=left_join(data.frame(Description=V(ig)$name),groups,by="Description")
V(ig)$color=groups$group#set color to group manually, wont work automatically

sizes=do.call(rbind,enriched)
sizes=sizes%>%filter(subtype=="Her2")%>%dplyr::select(subtype,Description,geneID)%>%
  separate_rows(geneID,sep="/",convert = T)%>%distinct()%>%count(subtype,Description)
sizes=left_join(data.frame(Description=V(ig)$name),sizes,by="Description")
V(ig)$size=sizes$n

createNetworkFromIgraph(ig)
setNodeSizeMapping(table.column="size",
    mapping.type="continuous",
    table.column.values=range(V(ig)$size),
    sizes=c(30,100))#fix it by hand
